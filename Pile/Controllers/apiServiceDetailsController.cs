...
namespace Pile.Controllers
{
    ...
    public class apiServiceDetailsController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> GetById(int id)
        {
            ...
            var customer = db.Customers.Single(x => x.CustomerId == id);
            var crews = db.Crews.Select(x => x.Employee); //db.Employees.Where(x => x.Status != "I" && x.Crew != null).OrderBy(e => e.Crew);
            ...
            var services = await db.Services.Where(x => x.Active == true || serviceIdsInUse.Contains(x.ServiceId)).OrderBy(x => x.DisplayOrder).ToListAsync();
